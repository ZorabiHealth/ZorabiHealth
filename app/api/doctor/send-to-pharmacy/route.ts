  ...
  try {
    ...
    const { data: order, error: orderErr } = await supabaseAdmin
      ...
      .insert({
        ...
        status: "pending",
        total_amount: prescription.prescription_items?.length ?? 0,
        delivery_address: "",
      ...
      .single();
    if (orderErr) {
      console.error("[send-to-pharmacy]", orderErr);
    ...
    await supabaseAdmin.from("notifications").insert({
      ...
      body: `New order #${trackingId} ready for fulfillment.`,
    ...
    return NextResponse.json(
      { orderId: order.id, trackingId, pharmacyName: pharmacy.business_name, estimatedDelivery },
      { status: 201 }
    );
  } catch (err) {
