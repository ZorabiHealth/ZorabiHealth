...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let query = supabase.from("pharmacy_profiles").select("id, business_name, address, phone, rating, delivery_radius_km, pincode").eq("is_active", true).eq("is_verified", true).order("rating", { ascending: false });
