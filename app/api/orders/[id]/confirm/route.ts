...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!deliveryAddress || !city || !pincode || !phone)
      return NextResponse.json(
        { error: "deliveryAddress, city, pincode, and phone are required" },
        { status: 400 }
      );
    const { data: order, error: orderErr } = await supabase
      .from("orders")
      .select("id, status, patient_id, pharmacy_id")
      .eq("id", id)
    ...
    if (order.status !== "pending")
      return NextResponse.json(
        { error: `Cannot confirm from status: ${order.status}` },
        { status: 422 }
      );
    ...
    await supabaseAdmin
      .from("orders")
      .update({ delivery_address: fullAddress, status: "confirmed", updated_at: now })
      .eq("id", id);
    await supabaseAdmin.from("order_events").insert({
      order_id: id,
      status: "confirmed",
      note: `Patient confirmed delivery to: ${fullAddress}`,
      timestamp: now,
    });
    if (order.pharmacy_id) {
      ...
      if (pharmacy)
        await supabaseAdmin.from("notifications").insert({
          ...
          body: "Patient confirmed delivery address.",
