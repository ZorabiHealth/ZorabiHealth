...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data: storeOrder } = await supabaseAdmin
      .from("store_orders")
      .select("id, status, pharmacy_id, user_id")
      .eq("id", id)
    ...
    if (storeOrder) {
      ...
      if (!allowed.includes(status.toLowerCase()))
        return NextResponse.json(
          { error: `Cannot transition from ${storeOrder.status} to ${status}` },
          { status: 422 }
        );
      await supabaseAdmin
        .from("store_orders")
        .update({ status: status.toUpperCase(), updated_at: now })
        .eq("id", id);
      await supabaseAdmin
        .from("store_order_events")
        .insert({ order_id: id, status: status.toUpperCase(), note: note ?? null, timestamp: now });
    ...
    const { data: rxOrder } = await supabaseAdmin
      .from("orders")
      .select("id, status, pharmacy_id, patient_id")
      .eq("id", id)
      .eq("pharmacy_id", profile.id)
      .maybeSingle();
    ...
    if (!allowed.includes(status.toLowerCase()))
      return NextResponse.json(
        { error: `Cannot transition from ${rxOrder.status} to ${status}` },
        { status: 422 }
      );
    await supabaseAdmin
      .from("orders")
      .update({ status: status.toLowerCase(), updated_at: now })
      .eq("id", id);
    await supabaseAdmin
      .from("order_events")
      .insert({ order_id: id, status: status.toLowerCase(), note: note ?? null, timestamp: now });
    await supabaseAdmin.from("notifications").insert({
      user_id: rxOrder.patient_id,
      title: "Prescription Order Update",
      body: `Your prescription order status: ${status}`,
      category: "order_update",
      created_at: now,
    });
    return NextResponse.json({ success: true, orderId: id, status });
