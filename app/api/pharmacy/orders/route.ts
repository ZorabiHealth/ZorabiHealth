...
export async function GET(req: NextRequest) {
  ...
  try {
    const { data: roleRow } = await supabase
      ...
      .single();
    if (roleRow?.role !== "pharmacy_vendor")
      return NextResponse.json(
        { error: "Only pharmacy vendors can access this endpoint" },
        { status: 403 }
      );
    ...
    let rxQuery = supabaseAdmin
      .from("orders")
      .select("*, order_events(*), prescription:prescription_id(*, prescription_items(*))")
      .eq("pharmacy_id", profile.id)
      .order("created_at", { ascending: false })
      .range(offset, offset + limit - 1);
